...
- ❌ 不会主动调用自己的能力（有枪不用）
- ❌ 不会把复杂任务拆成多段（一口气干到超token）
- ❌ 不会拒绝降级实现（输出示例/示意/占位符糊弄你）
- ❌ 没有长程记忆纠偏（做50轮对话后忘了最初目标）
- ❌ 没有任务队列管理（并行最多3个）

这个增强包把这五个问题全干了。**不是写规则让LLM自觉遵守，是改底层代码强制它执行。**
...
**Before**: 做了100轮，方向早偏了  
**After**: `gear_enforcer` 每5段调用 `meta_thinker_auto()` 检测语义漂移。漂移分数>0.5时告警

...
class AIAgent:
    ...
    def run_conversation(
        ...
        # ════════════════════════════════════════════════════════════
        # [PLUGIN-MANAGER] Hermes 系统增强插件矩阵 — 可插拔, 零侵入, 安全降级
        # ════════════════════════════════════════════════════════════
        # 管理16+个子插件: 武器强制调用/任务分解/一致性自检/复盘反思/
        #   记忆提取/检查点保存/CaMeL安全护栏/状态反馈/自动修复等
        # 引用失败: 完全跳过, 不影响原始对话流程
        # 异常: 被try-except捕获, 不抛到run_agent.py
        # ════════════════════════════════════════════════════════════
        _plugin_force_context = None
        try:
            import importlib
            _pm_path = os.path.join(os.path.dirname(get_hermes_home()), "scripts", "agent_enhancement_manager.py")
            if os.path.exists(_pm_path):
                _pm_spec = importlib.util.spec_from_file_location("agent_enhancement_manager", _pm_path)
                if _pm_spec and _pm_spec.loader:
                    ...
                    if hasattr(_pm_mod, 'safe_hook_pre_conversation'):
                        _plugin_force_context = _pm_mod.safe_hook_pre_conversation(self, user_message)
                        if _plugin_force_context:
                            logger.info(f"[PLUGIN-MANAGER] ✅ 强制上下文已生成 ({len(_plugin_force_context)} chars)")
        except Exception:
            pass
        # ════════════════════════════════════════════════════════════
        # [PLUGIN-MANAGER 结束]
        ...
        # ════════════════════════════════════════════════════════════
        # [PLUGIN-MANAGER] 对话后钩子 — 执行44个post插件
        # ════════════════════════════════════════════════════════════
        # 执行复盘/记忆/安全/反馈等全部post类型增强
        # 引用失败: 完全跳过, 不影响返回结果
        try:
            import importlib
            _pm_path = os.path.join(os.path.dirname(get_hermes_home()), "scripts", "agent_enhancement_manager.py")
            if os.path.exists(_pm_path):
                _pm_spec = importlib.util.spec_from_file_location("agent_enhancement_manager_post", _pm_path)
                if _pm_spec and _pm_spec.loader:
                    _pm_mod_post = importlib.util.module_from_spec(_pm_spec)
                    _pm_spec.loader.exec_module(_pm_mod_post)
                    if hasattr(_pm_mod_post, 'safe_hook_post_conversation'):
                        _pm_mod_post.safe_hook_post_conversation(self, final_response, user_message)
                        logger.info("[PLUGIN-MANAGER] ✅ 对话后增强完成 (44个post插件)")
        except Exception:
            pass
        # ════════════════════════════════════════════════════════════
        # [PLUGIN-MANAGER] post钩子结束
        # ════════════════════════════════════════════════════════════
